...
(function() {
	...
	function createBackup(x) {
		...
		ret = '';
		for (let i=0; i<x.length; i++) ret += x[i] + (i<x.length-1?'\n':'');
		return ret;
	}
	...
	function backgroundBackup(callback) {
		...
		let callbackS = function (r) {
            ...
            s.getElementById('serve_hidpi').checked = localStorage.serve_hidpi;
            s.getElementById('serve_webm').checked = localStorage.serve_webm;
            s.getElementById('webm').checked = localStorage.webm;
            s.getElementById('hide_uploader').checked = localStorage.hide_uploader;
            s.getElementById('dont_track').checked = localStorage.dont_track;
			process();
		};
	...
	function backup() {
		...
		let container = document.getElementById('user_watched_images_exclude_str');
		let x = container.value.split('\n');
		container.value = createBackup(x);
