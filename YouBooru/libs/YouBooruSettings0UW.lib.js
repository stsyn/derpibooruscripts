...
function start() {
  ...
  function register(onlyLibs, win) {
    ...
    if (!win._YDB_public.funcs) win._YDB_public.funcs = {};
    ...
    if (!win._YDB_public.funcs.getApiKey && features.storage) win._YDB_public.funcs.getApiKey = () => {
      return getApiKey();
    }
    _YDB_S_OuterScope.getApiKey = () => getApiKey();
  }
