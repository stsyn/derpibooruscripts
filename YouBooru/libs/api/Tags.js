...
(function() {
    ...
    YDB_api.tagSearch = async function (search, options) {
        const enviroment = YDB_api.getEnviroment();
        if (enviroment !== 'philomena') {
            throw new Error('Unsupported enviroment: ' + enviroment);
        }

        const url = `/api/v1/json/search/tags?per_page=50&q=${search}&page=${options?.page ?? 1}`;
        ...
        content.tags = content.tags.map(parseTag);

        return content;
    }

    async function* longTagSearchIterator(query, options) {
        let notEmpty = true;
        for (let page = 1; notEmpty; page++) {
            const data = await YDB_api.tagSearch(query, { ...options, page });
            if (data.tags.length < 50) notEmpty = false;
            yield data;
        }
    }

    YDB_api.performLongTagSearch = async function (query, options) {
        const data = {tags: []};
        const iterator = longTagSearchIterator(query, options)
        for await (let content of iterator) {
            data.tags = data.tags.concat(content.images);
            data.total = content.total;
        }
        return data;
    }

    YDB_api.longTagSearch = function (query, options) {
        return longTagSearchIterator(query, options);
    }
    ...
    YDB_api.searchTags = async function(search, options = {}) {
